...
model = joblib.load('assets/xgb_final_weighted.pkl')
selected_features = pd.read_csv("assets/final_features_weighted.csv")["Feature"].tolist()
...
# User input fields -- this section is where the customer can see whether they are credit worthy or not
# perhaps we could have another tab which is used by the banker, and have access to the full database (test.csv)
# which would be used to make the decision
age = st.number_input(
    "Age",
    min_value=18,
    max_value=100,
